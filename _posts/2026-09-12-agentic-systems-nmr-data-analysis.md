...
markdown_content: |-
  ...
  The broader opportunity is an NMR agent that combines such tools with explicit planning, evidence tracking, critique, and chemist review. Spectra is Rombo AI’s AI agent for NMR analysis and is distinct from Rombo AI Platform. Any API access is an interface to Spectra, not a separate product. Product boundaries and available integrations should be confirmed from current documentation rather than inferred from the architecture described here.
